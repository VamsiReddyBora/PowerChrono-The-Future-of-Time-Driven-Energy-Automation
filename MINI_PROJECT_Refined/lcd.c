...
extern ui32 device_on_time;
extern ui32 device_off_time;
		...
		if(keypress != '\0'){
			...
			switch(keypress){
				...
				case '=': DOM = date_count;     // Save new date to globals
									MONTH = month_count;
									if(year_count != 2025){ //Giving Warning, but allowing 
...
i32 edit_on_time(){
	...
	while(timeout--){                            // UI loop
		...
		switch(keypress){
			...
			case '=': if((!(device_off_hour) && !(device_off_min)) || ((hour_count <= device_off_hour) && (min_count < device_off_min))){
									device_on_hour = hour_count; // Save ON time fields
	...
	while(timeout--){                            // UI loop
		...
		switch(keypress){
			...
			case '=': 
								...
								device_off_time = (hour_count * 60) + min_count;
			
								if((!(device_on_hour) && !(device_on_min))){
								...
								else if(device_on_time < device_off_time){
									device_off_hour = hour_count;
									device_off_min = min_count;
									device_off_sec = sec_count;
									COMMAND(CLEAR_LCD);
